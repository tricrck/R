...
# plot mean TotalCharges
ggplot(plotdata, 
       aes(x = Churn, 
           y = mean_charges)) +
  geom_bar(stat = "identity")

# plot mean Total Charges in a more attractive fashion
...
# plot the means and standard errors by Gender
...
ggplot(data, 
  ...
  geom_quasirandom(alpha = 0.7,
                   size = 1.5) + 
  scale_y_continuous(label = dollar) +
  labs(title = "Telecom Dataset Total Charges by Churn", 
       subtitle = "Telecom Dataset Churn Prediction",
       x = "",
       y = "") +
  theme_minimal() +
...
# subset Electronic check Payment Methods in who Churn.
